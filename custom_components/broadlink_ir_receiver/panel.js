class BroadlinkIRPanel extends HTMLElement {
  ...
  _renderSelector() {
    ...
    const freqTag = cur.type === "rf" && cur.rf_frequency ? ` <span style="font-size:10px;color:var(--secondary-text-color)">${Number(cur.rf_frequency).toFixed(2)} MHz</span>` : "";
  ...
  _stepCapture() {
    ...
    if (w.capturing || w.rfWaiting) {
      ...
      if (isRFcap && phase === 2 && w.rfWaiting) {
        ...
        const freqStr = freq ? `<b>${Number(freq).toFixed(2)} MHz</b>` : "unknown";
